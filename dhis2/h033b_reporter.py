...
class H033B_Reporter(object):
    ...
    def _exclude_soft_approved_submissions_from_removal(self, reported_submissions_ids):
        ...
        if soft_approved_submissions:
            soft_approved_ids = [
                reported_submissions_ids.remove(submission.id) for submission in soft_approved_submissions
                if submission.submission_values().order_by('modified')[0].modified >
                Dhis2_Reports_Submissions_Log.objects.filter(
    ...
    def submit_and_log_task_now(self, submissions):
        ...
        failure = Dhis2_Reports_Submissions_Log.objects.filter(
            task_id=self.current_task, result=Dhis2_Reports_Submissions_Log.FAILED)
        ...
        self.log_submission_finished(
            submission_count=len(submissions), status=status, description=description)
